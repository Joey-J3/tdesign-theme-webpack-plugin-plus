...
A webpack plugin designed to customize styles by generating color palette, which overrides the default styles of the tdesign light theme.

## Intro

```bash
npm install
npm run install # install example modules
npm run dev:example
```
...
```js
const ThemeWebpackPlugin = require('tdesign-theme-webpack-plugin-plus');
